...
import { HistoryGroup, DecodedEntry } from './types.ts';
...
export default function History({ history, clearHistory }: Props): ReactElement {
    ...
    return (
        <div className="history-container">
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    ...
                    {history.length > 0 ? (
                        history.map((group, index) => (
                            <Fragment key={index}>
                                <tr>
                                    <td colSpan={2} style={{ background: '#f0f0f0', textAlign: 'left', fontWeight: 'bold' }}>
                                        {group.date}
                                    </td>
                                </tr>
                                {group.items.map((item, idx) => (
                                    <tr
                                        ...
                                        style={{
                                            ...
                                            background: selectedJson === item ? '#ccf2ff' : 'transparent'
                                        }}
                                    >
                                        <td>{new Date(item.date).toLocaleString()}</td>
                                        <td>{JSON.stringify(item.json, null, 2).slice(0, 30)}...</td>
                                    </tr>
                                ))}
                            </Fragment>
                        ))
            ...
            <div className="json-display json-output-container">
                <h3>Selected JSON</h3>
                {selectedJson ? (
                    <ReactJson name={null}  src={selectedJson.json} theme="monokai" collapsed={2} />
                ) : (
